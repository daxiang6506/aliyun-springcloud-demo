...
docker tag $(docker build -t ${REGPREFIX}gateway -q .) ${REGPREFIX}gateway:$(date -ju "+%Y%m%d-%H%M%S")
cd -
...
docker tag $(docker build -t ${REGPREFIX}foo -q .) ${REGPREFIX}foo:$(date -ju "+%Y%m%d-%H%M%S")
cd -
...
docker tag $(docker build -t ${REGPREFIX}bar -q .) ${REGPREFIX}bar:$(date -ju "+%Y%m%d-%H%M%S")
cd -
...
docker tag $(docker build -t ${REGPREFIX}foobar -q .) ${REGPREFIX}foobar:$(date -ju "+%Y%m%d-%H%M%S")
cd -
